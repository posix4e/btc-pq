// Local transaction-level proof rules, enforced with Core's linked-prevout
// Script interpreter plus a real RISC Zero receipt verifier. Not a network node.
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const CTransaction encoded = Read(argv[1]);
        Require(!encoded.IsCoinBase() && !encoded.vin.empty() && encoded.vin.size() <= 512, "input count");
        Require(argc == int(encoded.vin.size())+2, "one parent per input required");
        ...
        for (size_t i = 0; i < tx.vin.size(); ++i) {
            ...
            Require(MoneyRange(prevouts.back().nValue), "spent amount range");
            fee += prevouts.back().nValue;
        ...
        flags |= SCRIPT_VERIFY_BTC_PQ_COVENANT_DEMO | SCRIPT_VERIFY_BTC_PQ_SHRINCS_DEMO | SCRIPT_VERIFY_BTC_PQ_SHRINCS_PROOF_DEMO;
        CSHA256 claim;
