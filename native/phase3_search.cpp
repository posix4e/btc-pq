...
struct Recovery {
    ...
    std::vector<Bytes> Keys(const Hash& z) {
        Hash u1; std::vector<Bytes> out;
        if(!Scalar(z,u1)) return out;
        secp256k1_pubkey p;
    ...
    try {
        ...
        if(mode=="vectors") {
            ...
            for(const auto& value:req["digests"].getValues()) {
                ...
                UniValue keys(UniValue::VARR); unsigned b=0;
                for(const auto& key:recovery.Keys(z)) {
                    Hash h=DoubleHash(Bytes(1,0)); // placeholder replaced below
        ...
        for(;count<maximum;++count) {
            ...
            if(counter>=0x80000000ULL) { space_exhausted=true; break; }
        ...
        out.pushKV("budget_exhausted",passes==0 && !space_exhausted);
